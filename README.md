...
API 會開在 `http://localhost:8787`。另開一個終端機，在專案根目錄跑 `npm run dev`，Vite 會自動把 `/api` 轉到本機 API。
...
6. `npm run deploy`（會先 build 前台，再部署前台與 API）
...
| 正式站 | `cd worker && npm run deploy` | `/` | https://ryukyusurfbase.sssuni.com（Worker＋Static Assets，前台與 API 同一個 Worker） |
...
這兩步**只發生在 GitHub Pages 的 workflow 裡**。正式站則是另外用 `public/_headers` 加上 `X-Robots-Tag: noindex`：Kaito 驗收前先不讓搜尋引擎收錄。**正式開放時，要刪掉那兩行再部署。**
